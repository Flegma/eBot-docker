...
printf "This script configures the .env file. Config files are generated automatically by containers at startup.\r\n"

cp .env .env.bak
if yesNo "Would you like generate a new random websocket secret?"
then
  NEW_WEBSOCKET_SECRET_KEY=$(generatePassword)
  sedi "s#WEBSOCKET_SECRET_KEY=.*#WEBSOCKET_SECRET_KEY=${NEW_WEBSOCKET_SECRET_KEY}#g" \
      "$(dirname "$0")/.env"
fi

if yesNo "Would you like generate new mysql random passwords?"
then
  NEW_MYSQL_ROOT_PASSWORD=$(generatePassword)
  NEW_MYSQL_PASSWORD=$(generatePassword)
  sedi "s#MYSQL_ROOT_PASSWORD=.*#MYSQL_ROOT_PASSWORD=${NEW_MYSQL_ROOT_PASSWORD}#g" \
      "$(dirname "$0")/.env"
  sedi "s#MYSQL_PASSWORD=.*#MYSQL_PASSWORD=${NEW_MYSQL_PASSWORD}#g" \
      "$(dirname "$0")/.env"
fi
...
sedi "s#EBOT_ADMIN_LOGIN=.*#EBOT_ADMIN_LOGIN=${EBOT_ADMIN_LOGIN}#g" \
    "$(dirname "$0")/.env"
sedi "s#EBOT_ADMIN_PASSWORD=.*#EBOT_ADMIN_PASSWORD=${EBOT_ADMIN_PASSWORD}#g" \
    "$(dirname "$0")/.env"
sedi "s#EBOT_ADMIN_EMAIL=.*#EBOT_ADMIN_EMAIL=${EBOT_ADMIN_EMAIL}#g" \
    "$(dirname "$0")/.env"
...
sedi "s#LOG_ADDRESS_SERVER=.*#LOG_ADDRESS_SERVER=${LOG_ADDRESS_SERVER}#g" \
    "$(dirname "$0")/.env"
...
sedi "s#EBOT_IP=.*#EBOT_IP=${EBOT_IP}#g" \
    "$(dirname "$0")/.env"
...
then
   ...
   sedi "s#WEBSOCKET_URL=.*#WEBSOCKET_URL=${WEBSOCKET_URL}#g" \
       "$(dirname "$0")/.env"
   ...
   sleep 5
    ...
    then
        ...
        sedi "s#WEBSOCKET_URL=.*#WEBSOCKET_URL=http://${EBOT_IP}:12360#g" \
            "$(dirname "$0")/.env"
    fi
fi

printf "$green" "Setup complete! You can now run: docker compose up -d --build"
